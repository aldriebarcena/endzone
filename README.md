...
<p float="left">
  ...
  <img src="docs/screenshots/live-feed.png" width="220" alt="Live feed with real scoring plays" />
  <img src="docs/screenshots/import-league.png" width="220" alt="League import form" />
</p>

## What it does

A user signs in with Apple, imports their Sleeper fantasy league, and gets push notifications as their tracked NFL game happens — each notification carrying real per-player fantasy points, computed against their league's actual scoring settings, not a generic estimate.
...
checker Lambda (EventBridge, ~15min)
  ...
  → starts a Step Functions execution
      → poller Lambda loops every 90s (Step Functions Wait state)
          → diffs new scoring plays, enriches via ESPN, publishes to SQS
...
.venv/bin/python -m pytest        # 53 tests, no AWS credentials needed
