...
- **Supplementary provider for fantasy point computation:** ESPN's unofficial, undocumented site API (no key; game lookup via the `espnID` Tank01 already includes) gives structured per-play yardage and pass/rush play-type classification that Tank01 doesn't expose as structured fields — needed to compute real per-player fantasy points against a league's actual scoring settings. Accepted as a real dependency risk (could change or get blocked without notice) because Tank01 alone can't support the app's core value prop otherwise. See `backend/src/fantasy_points.py` and PROJECT_PLAN.md's open questions for what's built vs. still unverified.
